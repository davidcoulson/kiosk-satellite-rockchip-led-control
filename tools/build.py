...
with tempfile.TemporaryDirectory(prefix='kiosk-plugin-') as temp:
    ...
    ndks = sorted((sdk_root / 'ndk').glob('*'), key=lambda p: tuple(int(v) for v in p.name.split('.') if v.isdigit()))
    if not ndks:
        raise SystemExit('Install the Android NDK before building this plugin.')
    compiler_root = ndks[-1] / 'toolchains/llvm/prebuilt/linux-x86_64/bin'
    native_files = {}
    for abi, target in [('arm64-v8a', 'aarch64-linux-android'), ('armeabi-v7a', 'armv7a-linux-androideabi'), ('x86_64', 'x86_64-linux-android')]:
        library = temp / abi / 'librockchip_led.so'
        library.parent.mkdir()
        subprocess.run([str(compiler_root / f'{target}24-clang'), '-shared', '-fPIC', '-Os', '-Wall', '-Wextra', '-Werror',
                        '-Wl,-z,max-page-size=16384', '-Wl,-soname,librockchip_led.so',
                        str(plugin / 'native/led.c'), '-o', str(library)], check=True)
        native_files[f'native/{abi}/librockchip_led.so'] = library.read_bytes()
